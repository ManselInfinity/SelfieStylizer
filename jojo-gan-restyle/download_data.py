...
def download_from_drive(file_name):
    ...
    print(f"download from ${file_name}")
    file_dst = os.path.join("models", file_name)
    if not os.path.exists(file_dst):
        gdown.download(id=drive_ids[file_name], output=file_dst, quiet=False)


def main():
    '''Downloads the facial landmarks, pretrained StyleGAN2, e4e and ReStyle models.'''
