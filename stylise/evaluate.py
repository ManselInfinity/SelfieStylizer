...
def parse_args():
    ...
    parser.add_argument("--input", default="temp.jpg", help="input image")
...
if __name__ == "__main__":
    ...
    # original_generator = Generator(1024, latent_dim, 8, 2).to(device)
    ckpt = torch.load(os.path.join('models', args.model_name+'.pt'),
                      map_location=lambda storage, loc: storage)
    ...
    with torch.no_grad():
        ...
        torchvision.utils.save_image(utils.make_grid(
            my_output, normalize=True, range=(-1, 1)), "results/temp"+".png")
